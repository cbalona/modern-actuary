...
<svelte:head>
  ...
  <meta
    name="description"
    content="Technical insights on actuarial science, data science, and modern software development."
  />
</svelte:head>

<section>
  <header class="mb-8 pb-4 border-gray-200 border-b">
    <h1 class="font-semibold text-3xl md:text-4xl tracking-tight">The Journal</h1>
    <a
      href={resolve("/archive")}
      class="font-sans text-ink-muted hover:text-accent text-base transition-colors"
    >
      View Archive &rarr;
    </a>
  </header>
  ...
  <ul class="space-y-8">
    {#if data.journalEntries.length > 0}
      {#each data.journalEntries as entry (entry.slug)}
        <li>
          <a href={resolve(`/journal/${entry.slug}`)} class="group block">
