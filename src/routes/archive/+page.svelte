...
<section>
  <header class="mb-8 pb-4 border-gray-200 border-b">
    <h1 class="font-semibold text-3xl md:text-4xl tracking-tight">Archive</h1>
          ...
          <a href={resolve(`/journal/${entry.slug}`)} class="group block">
            <h2 class="font-semibold group-hover:text-accent text-2xl transition-colors">
              {#if entry.metadata.deprecated}
                <span
                  class="rounded-full bg-orange-200 px-3 py-1 align-middle text-sm font-semibold uppercase tracking-wider text-orange-800"
                >
                  Deprecated
                </span>
              {/if}
